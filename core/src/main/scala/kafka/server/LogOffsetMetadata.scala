 ...
 */
case class LogOffsetMetadata(messageOffset: Long, // 消息偏移量
                             segmentBaseOffset: Long = Log.UnknownOffset, // 段基偏移量
                             relativePositionInSegment: Int = LogOffsetMetadata.UnknownFilePosition) { // 该位移值所在日志段的实际物理磁盘位置
